...
namespace CommonValueConverters.Converters
{
    ...
    public class MultiConverterGroup : IMultiValueConverter
    {
        ...
        public object Convert(object[] values, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            if (this.Steps[this.Steps.Count - 1].Converters.Count != 1)
                throw new InvalidOperationException("The final step in a MultiConverterGroup must have a single converter added to it.");

            foreach (var step in this.Steps)
            {
                if (step.Converters.Count == 0)
                    throw new InvalidOperationException("Each step in a MultiConverterGroup must have at least one converter added to it.");
        ...
        public object[] ConvertBack(object value, Type[] targetTypes, object parameter, CultureInfo culture)
        {
            ...
            for (var i = this.Steps.Count - 1; i >= 0; --i)
            {
                ...
                if (step.Converters.Count == 0)
                    throw new System.InvalidOperationException("Each step in a MultiConverterGroup must have at least one converter added to it.");

                if (step.Converters.Count != stepValues.Length)
                    throw new System.InvalidOperationException(string.Format("Step with index {0} produced {1} values but the subsequent step (index {2}) requires {3} values.", i + 1, stepValues.Length, i, step.Converters.Count));
